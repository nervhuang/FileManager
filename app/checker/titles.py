...
def attribution_key(parsed):
    """把「這本書掛在誰名下」壓成一個比對鍵：`[社團 (作者)]` 那一格。

    核心標題相同、掛的社團與作者也相同，就是同一本書——不管是從哪一位作者的
    tag 掃到的。合志本與商業誌正是靠這個認出來：`[フェチズムポケット (よろず)]`
    在每一位參與作者底下都一模一樣，`COMIC ExE 69` 這種商業誌則是兩邊都沒有
    社團括號。反過來，核心標題**碰巧**相同的兩本書（實測有 `C108 おまけ本`、
    `fanbox`、`メイ`）掛的名字不同，就不會被誤認成同一本。

    與 `core_key` 用同一套正規化，因為同一個名字在不同來源的標點與空白幾乎必定
    不同（實測 `四万十川 、にやすけ` 與 `四万十川、にやすけ`）。社團與作者接在
    一起壓成一個鍵，邊界因此消失，但它從不單獨使用——永遠與核心標題一起比對。
    """
    return core_key((parsed.get('circle') or '') + (parsed.get('artist') or ''))
