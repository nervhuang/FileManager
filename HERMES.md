...
| 參數 | 預設 | 說明 |
|---|---|---|
| `name` | — | 作者或團體名稱（也比對別名與英文名稱） |
| `type` | — | `author` / `circle`，用於消歧義 |
| `id` | 0 | 直接指定實體 id |
| `limit` | 200 | 最多回傳幾筆 |

把該實體的**名稱與所有別名**組成 OR 查詢丟進同一套搜尋管線，因此結果與使用者在程式裡點該項目看到的完全一致（英文名稱不算進這個查詢，純中繼資料）。`name` 可以直接給英文名稱查，一樣能找到對應的實體。
...
- `note`
- `english_name`——該作者/團體在網站上查詢用的英文名稱，**單一字串**（一對一，不是陣列）。純中繼資料，不會併入本機檔案搜尋，但 `fm_authors_list` 的 `keyword` 與 `fm_match_author`／`fm_authors_link` 的名稱參數都找得到它
